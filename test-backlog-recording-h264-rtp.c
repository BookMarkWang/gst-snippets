 ...
 *
 * Here the h264 input data comes in over RTP.
 *
 ...
 *
 * We replace the muxer here after ever recording because of a bug in
 * mpegtsmux, it doesn't reset itself properly when being set to NULL and
 * back to PLAYING and vlc complains about stuff being messed up on the
...
app_update_filesink_location (RecordApp * app)
{
  ...
  fn = g_strdup_printf ("/tmp/test-%03d.ts", app->chunk_count++);
    ...
    case GST_MESSAGE_ELEMENT:{
      ...
      if (gst_structure_has_name (s, "GstBinForwarded")) {
        ...
        if (GST_MESSAGE_TYPE (forward_msg) == GST_MESSAGE_EOS) {
          ...
          gst_bin_remove (GST_BIN (app->pipeline), app->muxer);
          app->muxer = gst_element_factory_make ("mpegtsmux", NULL); 
          gst_bin_add (GST_BIN (app->pipeline), app->muxer);
          gst_element_link_many (app->vrecq, app->muxer, app->filesink, NULL);
...
main (int argc, char **argv)
{
  ...
  app.pipeline =
      gst_parse_launch (" udp://172.23.3.97:50000 ! application/x-rtp,encoding-name=H264,media=video,clock-rate=90000 ! rtpjitterbuffer ! rtph264depay ! tee name=vtee "
      "vtee. ! queue ! avdec_h264 ! videoconvert ! videoscale ! autovideosink "
      "vtee. ! queue name=vrecq ! mpegtsmux name=mux ! filesink async=false name=filesink",
      NULL);
